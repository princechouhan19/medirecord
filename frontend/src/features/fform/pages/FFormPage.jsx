...
export default function FFormPage() {
  ...
  const handleAISuggest = async () => {
    ...
    setTimeout(() => {
      ...
      const suggestion = match
        ? suggestions[match]
        : { code: "R69", diag: "Illness, unspecified" };
      setForm((f) => ({
        ...f,
  ...
  const sv = (key) => ({
    ...
    onChange: (e) =>
      setForm({ ...form, vitals: { ...form.vitals, [key]: e.target.value } }),
  });
    ...
    <div className="fform-page">
      ...
      <div className="fform-tabs">
        {TABS.map((tab) => (
          <button
            key={tab}
            className={`fform-tab ${activeTab === tab ? "fform-tab--active" : ""}`}
            onClick={() => setActiveTab(tab)}
          >
            {tab}
            {tab === "Saved Forms" && forms.length > 0 && (
              <span className="fform-tab__count">{forms.length}</span>
            )}
          </button>
        ))}
      </div>
        ...
        <form onSubmit={handleSubmit} className="fform-layout">
          ...
          <div className="card fform-section">
            ...
            <div className="fform-row-2">
              ...
              <div className="form-group">
                ...
                <input
                  {...sf("currentMedications")}
                  placeholder="Ongoing medications and dosage"
                />
              </div>
            </div>
          </div>
          ...
          <div className="card fform-section">
            ...
            <div className="form-group" style={{ marginTop: "16px" }}>
              ...
              <textarea
                {...sf("physicalExamination")}
                rows={3}
                placeholder="General appearance, system-wise findings..."
              />
            </div>
            <div className="form-group">
              ...
              <textarea
                {...sf("investigations")}
                rows={3}
                placeholder="Tests ordered, lab results, imaging findings..."
              />
            </div>
          </div>
          ...
          <div className="card fform-section">
            ...
            <div className="form-group">
              ...
              <div className="icd-wrap">
                <input
                  {...sf("icdCode")}
                  placeholder="e.g. Z34.0 — AI will auto-suggest"
                />
                <button
                  type="button"
            ...
            <div className="form-group">
              ...
              <textarea
                {...sf("treatmentPlan")}
                rows={3}
                placeholder="Management plan, procedures..."
              />
            </div>
            <div className="form-group">
            ...
            <div className="fform-row-2">
              <div className="form-group">
                ...
                <input type="date" {...sf("followUpDate")} />
              </div>
              <div className="form-group">
      ...
      {activeTab === "Saved Forms" && (
        <div className="fform-saved">
          <div className="search-bar" style={{ marginBottom: "16px" }}>
            ...
            <input
              placeholder="Search by patient, form number, or complaint..."
              value={searchForms}
              ...
              <table className="data-table">
                ...
                <tbody>
                  {filteredForms.map((f) => (
                    <tr key={f._id}>
                      <td>
                        <span className="form-number">{f.formNumber}</span>
                      </td>
                      <td>
                        <div className="td-name">{f.patient?.name}</div>
                        <div className="td-muted">
                          {f.patient?.age} yrs, {f.patient?.gender}
                        </div>
                      </td>
                  ...
                  {filteredForms.length === 0 && (
                    <tr>
                      <td colSpan={7} className="empty-row">
                        {forms.length === 0
                          ? "No forms saved yet. Create your first F-Form."
      ...
      {viewForm && (
        <FFormView form={viewForm} onClose={() => setViewForm(null)} />
      )}
    </div>
  );
}
