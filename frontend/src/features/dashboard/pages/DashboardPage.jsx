...
export default function DashboardPage() {
  ...
  useEffect(() => {
    gsap.fromTo(headerRef.current, { y: -20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: 'power2.out' })
    gsap.fromTo('.stat-card', { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, stagger: 0.1, ease: 'power2.out', delay: 0.2 })
  ...
  return (
    <div className="dashboard">
      ...
      <div className="dashboard__stats" ref={statsRef}>
        <StatCard label="Total Patients" value={patientStats.total} sub={`+${patientStats.thisWeek} this week`} icon="👥" />
        <StatCard label="Reports Generated" value={reports.count} sub="All up to date" icon="📄" />
        <StatCard label="Upcoming Visits" value={schedule.upcoming} sub="Next 7 days" icon="📅" />
        <StatCard label="Overdue Visits" value={schedule.overdue} sub="Needs attention" icon="⚠️" variant="danger" />
      </div>
      ...
      <div className="dashboard__panels">
        <div className="card dashboard__recent">
          <div className="panel-header">
            ...
            <Link to="/registration" className="panel-link">View all →</Link>
          </div>
          <div className="patient-list">
            ...
            {patients.length === 0 && <p className="empty-state">No patients yet. <Link to="/registration">Register one →</Link></p>}
          </div>
        </div>
        ...
        <div className="card dashboard__schedule">
          <div className="panel-header">
            ...
            <Link to="/tracking" className="panel-link">View all →</Link>
          </div>
          <div className="schedule-list">
            {trackings.map(t => (
              <div key={t._id} className={`schedule-row ${t.status === 'Overdue' ? 'schedule-row--overdue' : ''}`}>
                ...
                <div className="schedule-row__right">
                  {t.status === 'Overdue'
                    ...
                    : <span className="schedule-row__date">{new Date(t.nextVisit).toLocaleDateString('en-CA')}</span>}
                </div>
              </div>
            ))}
            {trackings.length === 0 && <p className="empty-state">No scheduled visits.</p>}
          </div>
        </div>
      </div>
    </div>
