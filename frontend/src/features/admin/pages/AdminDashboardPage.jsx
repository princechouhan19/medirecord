...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(".admin-stat", { y: 30, opacity: 0 }, { y: 0, opacity: 1, stagger: 0.1, delay: 0.2, duration: 0.5 })
  ...
  return (
    <div className="admin-page">
      ...
      <div className="admin-stats">
        {[
          { label: "Total Clinics", value: stats.totalClinics, icon: "🏥", color: "blue" },
          { label: "Active Clinics", value: stats.activeClinics, icon: "✅", color: "green" },
          { label: "Total Patients", value: stats.totalPatients, icon: "👥", color: "teal" },
      ...
      <div className="card admin-clinics-table">
        <div className="panel-header">
          ...
          <Link to="/admin/clinics" className="panel-link">View all →</Link>
        </div>
        <table className="data-table">
          <thead>
            <tr><th>CLINIC NAME</th><th>OWNER</th><th>PLAN</th><th>PATIENTS</th><th>STAFF</th><th>STATUS</th></tr>
          </thead>
          <tbody>
            {clinics.map((c) => (
              <tr key={c._id}>
                ...
                <td><span className={`badge badge--${c.plan === "enterprise" ? "warning" : "primary"}`}>{c.plan}</span></td>
                <td>{c._patientCount || 0}</td>
                <td>{c._staffCount || 0}</td>
                <td><span className={`badge badge--${c.isActive ? "success" : "danger"}`}>{c.isActive ? "Active" : "Suspended"}</span></td>
              </tr>
            ))}
            {clinics.length === 0 && <tr><td colSpan={6} className="empty-row">No clinics registered yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
